...
func IsPlanOptimizedStr(allowPlanOptimization bool) string {
	return fmt.Sprintf("Is plan optimized: %v", allowPlanOptimization)
}
...
func TestServiceSched_JobRegister_DistinctProperty_TaskGroup_Incr(t *testing.T) {
	for _, allowPlanOptimization := range []bool{false, true} {
		t.Run(IsPlanOptimizedStr(allowPlanOptimization), func(t *testing.T) {
			h := NewHarness(t, allowPlanOptimization)
			assert := assert.New(t)
